...
class ChatCompletionStreamParser {
  ...
  static Stream<LlamaCompletionChunk> parse({
    ...
    // 本地高吞吐模型下放宽 partial parse 节流，让正文 content 增量更快流出
    const plainPartialParseProbeInterval = 2;
    const signalDrivenPartialParseMinTokens = 1;
    const partialParseMinIntervalMs = 8;
    ...
    if (parseToolCallsEnabled) {
      ...
      LlamaLogger.instance.debug(
        '[PATCH-DEBUG] final: hasToolCalls=${parsed.hasToolCalls} '
        'contentLen=${parsed.content.length} sky=\n${fullOutput.substring(0, fullOutput.length > 120 ? 120 : fullOutput.length)}',
      );
  ...
  static bool _containsForcedOpenXmlToolEnvelope(
    ...
    if (thoughtEnd < 0) {
      // 思考尚未结束：只有当前内容**开头**明确是工具 XML 标签才抑制正文，
      // 避免普通正文（如示例里恰好含 '<function' 字样）被误判攒批、
      // 导致流式接口不逐 token 输出正文。
      final first = _firstNonWhitespaceIndex(text);
      if (first == null) return false;
      final lower = text.substring(first).toLowerCase();
      return lower.startsWith('<tool_call') ||
          lower.startsWith('<tool_calls') ||
          lower.startsWith('<function name') ||
          lower.startsWith('<function_call') ||
          lower.startsWith('<start_function_call');
    }
